...
const totalListens = GAMES_JSON.reduce((accumulator, game) => accumulator + game.backers, 0);
...
const totalgamesRaised = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString('en-US');
const totalGamescurr = GAMES_JSON.length;
const unfundedGamesText = unfundedGamesCount === 0
    ? 'All games have been fully funded!'
    : `There ${unfundedGamesCount === 1 ? 'is' : 'are'} ${unfundedGamesCount} unfunded game${unfundedGamesCount > 1 ? 's' : ''}. Consider supporting them!`;

const summaryText = `A total of $${totalgamesRaised} has been raised for ${totalGamescurr} games. ${unfundedGamesText}`;

// create a new DOM element containing the template string and append it to the description container
const unfundedGamesElement = document.createElement('p');
unfundedGamesElement.innerHTML = summaryText;
...
console.log(secondTopGame.name.split(' ')[0]);
...
secondGameElement.innerHTML = `Second Top Funded Game: ${secondTopGame.name}`;
